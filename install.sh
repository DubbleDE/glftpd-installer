...
function channel
{
	...
	then
		...
		while [[ -z $channelnr || "$channelnr" != [0-9] ]]
		do
			echo -n "How many channels do you require the bot to be in ? : " ; read channelnr
		done
	fi
...
function presystem
{
	...
	case $ask in
		...
		cp listaffils.sh $glroot/bin
		cp addaffil.sh $glroot/bin
		cp delaffil.sh $glroot/bin
